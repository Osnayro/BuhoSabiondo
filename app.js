...
const NicknameModal = {
    ...
    init() {
        ...
        const savedName = safeLocalGet('usuario_nombre', '');
        if (savedName) {
            this.hide();
            cargarYMostrarLotes();
        } else {
    ...
    async handleSubmit(event) {
        ...
        const resultado = await registrarOAutenticarUsuario(nombre, pin);
        ...
        safeLocalSet('usuario_nombre', nombre);
        safeLocalSet('usuario_pin', pin);
    ...
    solicitarReset() {
        if (confirm('¿Deseas cerrar sesión o cambiar tu apodo y PIN registrado?')) {
            localStorage.removeItem('usuario_nombre');
            localStorage.removeItem('usuario_pin');
            if (this.nicknameFieldEl) this.nicknameFieldEl.value = '';
...
function actualizarRachaDiaria() {
    ...
    const ultimaFecha = safeLocalGet('paes_last_active_date', '');
    let racha = parseInt(safeLocalGet('paes_daily_streak', '0'), 10);

    if (!ultimaFecha) {
        racha = 1;
    } else if (ultimaFecha !== hoy) {
        const fechaActual = new Date(hoy);
        const fechaPrev = new Date(ultimaFecha);
        const diffDias = Math.round((fechaActual - fechaPrev) / (1000 * 60 * 60 * 24));
    ...
    safeLocalSet('paes_last_active_date', hoy);
    safeLocalSet('paes_daily_streak', racha.toString());
    ...
    if (card) {
        ...
        card.innerHTML = `
            ...
            <div class="daily-streak-info">
                <div class="daily-streak-title">¡Racha Diaria: ${racha} día${racha > 1 ? 's' : ''}!</div>
                <div class="daily-streak-desc">${racha > 1 ? '¡Excelente constancia! Sigue practicando diariamente.' : '¡Primer día de tu racha! Practica cada día para superarte.'}</div>
            </div>
        `;
    }
}
    ...
    if (question.textKey && typeof paesTexts !== 'undefined' && paesTexts[question.textKey]) {
        ...
        lecturaContainer.innerHTML = `
            <div class="lectura-panel" id="lectura-panel-${question.textKey}">
                ...
                <div class="lectura-scroll-indicator">
                    <span>📖 Avance:</span>
                    <div class="lectura-scroll-bar-track">
                        <div class="lectura-scroll-bar-fill" id="lectura-scroll-fill"></div>
                    </div>
                    <span id="lectura-scroll-percent">0%</span>
                </div>
        ...
        const scrollTarget = lecturaContainer;
        if (scrollTarget) {
            scrollTarget.onscroll = () => {
                const max = scrollTarget.scrollHeight - scrollTarget.clientHeight;
                const current = scrollTarget.scrollTop;
                const pct = max > 0 ? Math.min(100, Math.max(0, Math.round((current / max) * 100))) : 100;
                const fill = document.getElementById('lectura-scroll-fill');
                const txt = document.getElementById('lectura-scroll-percent');
    ...
    if (demreContainer) {
        ...
        demreContainer.innerHTML = `
            <div class="paes-demre-card">
                ...
                <div class="paes-demre-desc">${estDemre.desc}</div>
            </div>
        `;
    }
