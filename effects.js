...
/**
 * PAES Challenge — Sabiondo Effects Manager v1.2.2
 * Efectos visuales académicos (Canvas 2D) + Sonidos + Toasts
 * Para "PAES Challenge: Desafío de Admisión Universitaria"
 * - Timeout de seguridad para splash
 * - Pausa automática del canvas cuando la pestaña no es visible
 * - Carga optimizada de sonidos
...
class ContiEffectsManager {
    ...
    constructor(config = {}) {
        ...
        this._setupReducedMotionCheck();

        console.log('🦉 Sabiondo Effects Manager v1.2.2 listo');
    ...
    _setupReducedMotionCheck() {
        const prefersReducedMotion = window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)');
        if (prefersReducedMotion && prefersReducedMotion.matches) {
            this.maxParticles = 50; // reducir partículas si el usuario prefiere menos movimiento
        }
    }
    ...
    triggerStarExplosion(x, y, count = 15) {
        if (!this.canvas || this._visibilityHidden) return;
    ...
    triggerStarRain(count = 30) {
        ...
        for (let i = 0; i < count; i++) {
            setTimeout(() => {
                if (this._visibilityHidden) return;
                this.particles.push({
                    type: 'star',
    ...
    triggerConfettiAcademico(duration = 2000, density = 2) {
        if (!this.canvas || this._visibilityHidden) return;
        const startTime = performance.now();
        const colors = this.colors.confetti;
        const spawn = (now) => {
            if (now - startTime > duration || this._visibilityHidden) return;
    ...
    _preloadSounds(onProgress) {
        ...
        // Timeout de seguridad: ocultar splash después de 8 segundos aunque no carguen sonidos
        this._splashTimeout = setTimeout(() => {
            if (!this.audioLoaded && !this.audioLoadError) {
                console.warn('⏱️ Timeout de splash: ocultando por seguridad');
                this.audioLoadError = true;
                this._onSoundLoadComplete();
            }
        }, 8000);
    ...
    _onSoundLoadComplete() {
        if (this._splashHandled) return;
        this._splashHandled = true;
